...
import { BookOpen, ChevronRight, Clock, GitBranch, Layers, Users } from "lucide-react";
...
export default function AboutPage() {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      {/* About sidebar */}
      <aside className="w-52 shrink-0 border-r border-gray-100 bg-gray-50 overflow-y-auto">
        ...
        <nav className="p-2">
          {SIDEBAR_ITEMS.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-left text-sm transition-colors hover:cursor-pointer ${
                active === item.id
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-12">
        ...
        <section id="team">
          ...
          <div className="grid grid-cols-1 gap-2">
            {TEAM.map((member, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-[#7b1113] flex items-center justify-center text-white text-xs font-bold">
        ...
        {/* Pipeline */}
        <section id="pipeline">
          <h2 className="text-xl font-bold text-gray-900 mb-4">System Pipeline</h2>
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              { step: "2", label: "Merge Memory",         desc: "Combine last 2 messages with current question for context" },
              ...
              { step: "6", label: "Return / Auto-Ticket", desc: "Return answer — if unanswerable, create DynamoDB ticket" },
            ].map((s, i, arr) => (
              ...
              >
                ...
                <div>
                  <p className={`text-sm font-semibold ${domain.active ? "text-white" : "text-gray-900"}`}>
                    {domain.name}
                    {domain.active && <span className="ml-2 text-xs font-normal bg-white/20 px-2 py-0.5 rounded-full">Our Group</span>}
                  </p>
                  <p className={`text-xs mt-0.5 ${domain.active ? "text-white/80" : "text-gray-500"}`}>{domain.desc}</p>
                </div>
              </div>
            ))}
          </div>
