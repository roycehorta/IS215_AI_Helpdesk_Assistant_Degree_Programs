...
const TicketsView: FC<Props> = ({ tickets, setTickets, onMenuOpen }) => {
  ...
  const handleSendReply = (_replyText: string, newStatus: Ticket["status"]) => {
    if (!selectedTicket) return;
    setTickets((prev) => prev.map((t) => t.id === selectedTicket.id ? { ...t, status: newStatus } : t));
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 md:px-8 py-4 md:py-5 bg-white border-b border-gray-200">
        <div className="flex items-center gap-3">
          <button onClick={onMenuOpen} className="md:hidden text-gray-500 hover:text-gray-700">
            <Menu className="h-5 w-5" />
          </button>
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-900">Ticket Management</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-2.5 py-2 text-xs border border-gray-200 rounded-lg bg-white hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-primary text-white text-xs font-medium rounded-lg hover:bg-primary/90 transition-colors"
          >
      ...
      {/* Status filter pills */}
      <div className="flex items-center gap-2 px-4 md:px-8 py-3 bg-white border-b border-gray-100 overflow-x-auto">
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 md:px-8 py-3 bg-white border-b border-gray-100">
        <div className="relative flex-1 min-w-[160px]">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        </div>
        <select
          value={sort}
          onChange={(e) => setSort(e.target.value)}
          className="text-xs border border-gray-200 rounded-lg px-2 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20"
        >
      ...
      {/* Error */}
      {error && (
        <div className="mx-4 md:mx-8 mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600">
          {error}
        </div>
      )}
                ...
                <table className="w-full text-left min-w-[600px]">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/80">
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider w-28">Ticket ID</th>
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Student</th>
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Category</th>
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Concern</th>
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider w-24">Date</th>
                      <th className="px-4 py-3.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider w-24">Status</th>
                    </tr>
                  </thead>
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <p className="text-sm font-medium text-gray-900 leading-tight">
                            {t.name || t.user.split("@")[0].replace(/[._]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                          </p>
                          <p className="text-[11px] text-gray-400 mt-0.5">{t.user}</p>
                        </td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4 max-w-[200px]">
                          <p className="text-sm text-gray-700 truncate">{t.details ?? "—"}</p>
                        </td>
                        <td className="px-4 py-4 text-xs text-gray-500 whitespace-nowrap">{t.date}</td>
                        <td className="px-4 py-4">
                          <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${STATUS_STYLES[t.status] ?? STATUS_STYLES["New"]}`}>
                            {t.status}
                          </span>
                        </td>
                      </tr>
            ...
            {filtered.length > 0 && (
              <div className="flex flex-wrap items-center justify-between mt-4 px-1 gap-2">
                <p className="text-xs text-gray-500">
                  Showing <span className="font-semibold text-gray-700">{startIndex}–{endIndex}</span> of{" "}
                  <span className="font-semibold text-gray-700">{filtered.length}</span> tickets
                </p>
                      ...
                      p === "..." ? (
                        ...
                        <button
                          ...
                          className={`min-w-[32px] h-8 px-2 rounded-lg text-sm font-medium transition-colors ${
                            page === p ? "bg-primary text-white" : "border border-gray-200 bg-white text-gray-600 hover:bg-gray-50"
                          }`}
                        >
                          {p}
                        </button>
      ...
      {selectedTicket && (
        <TicketModal ticket={selectedTicket} onClose={() => setSelectedTicket(null)} onSendReply={handleSendReply} />
      )}
      {isAdding && (
