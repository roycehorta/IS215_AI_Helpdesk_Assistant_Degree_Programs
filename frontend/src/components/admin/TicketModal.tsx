    ...
    try {
      const response = await fetch(import.meta.env.VITE_API_URL, {
        ...
        body: JSON.stringify({
          ...
          ticketId: ticket.id,
          toEmail: ticket.user,
          studentName: ticket.name || ticket.user.split("@")[0].replace(/[._]/g, " "),
          replyText,
        }),
      });
      ...
      setReplyText("");
      setSuccessToast(true); // Show success feedback
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/30 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        {/* Success toast */}
        {successToast && (
          <div className="mx-6 mt-4 flex items-center gap-3 p-3.5 bg-green-50 border border-green-200 rounded-xl text-sm text-green-800 animate-[fade-in_0.2s_ease-out]">
            <div className="shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-green-200">
              <svg
            ...
            <div className="flex-1">
              <span className="font-semibold">Reply sent successfully.</span>{" "}
              Status updated to{" "}
              <span className="font-bold capitalize">{localStatus}</span> and
              email delivered to{" "}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-4 justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
            <div>
              ...
              <p className="text-xs text-primary mt-0.5">{ticket.user}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">
                Category
            ...
            <div>
              ...
              <span
                className={`text-xs font-bold px-2.5 py-1 rounded-full border ${getStatusBadge(ticket.status)}`}
              >
                {ticket.status}
              </span>
            </div>
            <div>
              ...
              <select
                value={localStatus}
                onChange={(e) =>
                  setLocalStatus(e.target.value as Ticket["status"])
                }
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 text-xs text-gray-500 space-y-1">
            <p className="font-semibold text-gray-600">
              Helpdesk Admin — UPOU Helpdesk Team
            </p>
            <p>
              🌐{" "}
              <a
                href="https://upou.edu.ph"
            ...
            <p>
              📘{" "}
              <a
                href="https://facebook.com/upouofficial"
                className="text-primary hover:underline"
              >
                facebook.com/upouofficial
              </a>
            </p>
